...
class SimilarMovies extends React.Component {
    ...
    constructor(props) {
        ...
        if (props.type === 'Similar Movies') {
            fetch('https://api.themoviedb.org/3/movie/' + this.props.urlParams + '/similar?api_key=' + API_KEY + '&language=en-US')
                .then(res => res.json())
                .then(res => {
    ...
    render() {
        ...
        if (this.state.data === '') return ('');
            ...
            return (
                <div>
                    <Title level={3}><MovieIcon type="icon-movie" style={{ fontSize: "35px", paddingRight: "10px" }} /><h1><font color="gold">SIMILAR MOVIES</font></h1></Title>
                    <hr />
                    <br />
                    <Row gutter={[16, 16]}>
